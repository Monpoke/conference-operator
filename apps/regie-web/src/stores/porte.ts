 ...
 * créé.
 */
export const usePorteStore = defineStore('porte', () => {
  const amorce = ref<AmorcePortee>({
    portee: 'locale',
    roomId: null,
    salles: [],
    google: null,
  ...
  const porte = shallowRef<PorteRegie | null>(null)
  ...
  const choixDeSalle = computed(() => distante.value && roomId.value == null)

  function start(valeur: AmorcePortee): void {
    amorce.value = valeur
...
/**
  ...
  function ouvrir(abonnement: FluxEtat, options: Ouverture = ouverture): void {
    flux = abonnement
    ouverture = options
