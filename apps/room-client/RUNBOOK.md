...
| L'orateur entre dans le vif | **Début** (`D`) — le editing coupera tout ce qui précède |
| Moment marquant | **Marquer** (`M`) — saisir un libellé, ou laisser « Chapitre » |
| Le dernier mot est dit | **Fin** (`F`) — le editing coupera tout ce qui suit |
...
bornes de ce qui sera publié : le editing jette ce qui précède `D` et ce qui
...
il ne s'en ajoute jamais deux. Sans eux, le editing devra deviner les blancs
...
annonce « sidecar non écrit », le noter** : ce talk demandera un editing manuel.
...
2. Relancer un enregistrement. Prévenir le editing : ce talk sera en deux parties.
...
- **Rouge** (au-delà de 90 %) : OBS **perd probablement des images**, et rien
  ...
  l'encodage dans OBS-B, et prévenir le editing pour le talk déjà passé.
...
2. Ouvrir **🎞 Enregistrements** et contrôler ce qui a été produit. Chaque talk
   ...
   nécessaires au editing** — le `.json` contient les intervenants, les
