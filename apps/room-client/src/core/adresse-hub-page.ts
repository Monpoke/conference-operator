...
<form class="w-full max-w-[520px] rounded-2xl border border-edge bg-surface px-10 py-9" id="formulaire">
  ...
  <div class="mt-2.5 min-h-5 text-[13px] text-dim" id="etat"></div>
  <div class="min-h-5 text-[13px] text-alert" id="erreur"></div>
...
<script>
  ...
  async function tester() {
    ...
    const numero = ++sonde
    if (valeur === '') return afficher('text-dim', '')
    afficher('text-dim', 'vérification…')
    const joignable = await window.hub.tester(valeur)
    if (numero !== sonde) return
    afficher(
      joignable ? 'text-ok' : 'text-warn',
