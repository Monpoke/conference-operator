...
import { SuiviInterruption, formaterDuree } from '../src/core/interruptions.js'
...
describe('suivi des interruptions', () => {
  it('trace le premier échec, pas les suivants', () => {
    ...
    const temps = horloge()
    const suivi = new SuiviInterruption('flux de commandes', temps.lire)

    expect(suivi.echec().message).toMatch(/interrompu, nouvelle tentative/)
    for (let i = 0; i < 20; i += 1) {
      temps.avancer(2_000)
      if (i < 20) expect(suivi.echec().message).toBeNull()
    }
  })
  ...
  it('annonce le rétablissement, avec la durée réelle', () => {
    ...
    const temps = horloge()
    const suivi = new SuiviInterruption('flux du mur', temps.lire)
    suivi.echec()
    temps.avancer(8_000)
    suivi.echec()
  ...
  it('ne présente pas le premier raccordement comme un rétablissement', () => {
    const suivi = new SuiviInterruption('flux de commandes', horloge().lire)
    expect(suivi.retabli()).toBeNull()
  })
  ...
  it('met les durées sous une forme lisible', () => {
    expect(formaterDuree(9_400)).toBe('9 s')
    expect(formaterDuree(95_000)).toBe('1 min 35 s')
    expect(formaterDuree(3_900_000)).toBe('1 h 05')
  })
})
...
describe('format des lignes de journal', () => {
  ...
  it("porte l'heure locale en tête", () => {
    ...
    expect(formaterLigneJournal('info', 'hub rejoint', undefined, a(9, 5, 3))).toBe(
      '09:05:03 · hub rejoint',
    )
  })

  it('distingue les niveaux par un marqueur aligné', () => {
    expect(formaterLigneJournal('warn', 'flux coupé', undefined, a(14, 30, 0))).toContain(' ! ')
    expect(formaterLigneJournal('error', 'jeton refusé', undefined, a(14, 30, 0))).toContain(' ✕ ')
  })
  ...
  it("aplatit le cas courant d'un contexte à une seule information", () => {
    ...
    const ligne = formaterLigneJournal(
      'warn',
  ...
  it('garde les clés quand le contexte en porte plusieurs', () => {
    const ligne = formaterLigneJournal('info', 'assets préchargés', { downloaded: 34, failed: [] }, a(8, 0, 0))
