...
function deps(options: Partial<VodIndexDeps> = {}): VodIndexDeps {
  return { root: root, fs: nodeVodFs(), now: LATER, realNow: LATER, ...options }
}
 ...
 */
function video(name: string, bytes = 2_700_000_000): string {
...
function sidecar(name: string, patch: Partial<Sidecar> = {}): void {
  writeFileSync(join(root, name), JSON.stringify({ ...SIDECAR, ...patch }, null, 2))
}

/** A fake probe: what ffprobe would have read, without ffprobe. */
function probeFn(patch: Partial<VodProbe> = {}): (path: string) => Promise<VodProbe | null> {
  return async () => ({
    ouvert: true,
    durationMs: 45 * 60_000,
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'vod-'))
})
...
describe('listing the recordings', () => {
  it('pairs each video with its sidecar, newest first', async () => {
    ...
    utimesSync(join(root, '2026-10-30_track1_1000_honeyswamp.mkv'), new Date(1e9), new Date(1e9))
  ...
  it('ignores anything that is not a video', async () => {
    video('take.mkv')
    writeFileSync(join(root, 'notes.txt'), 'rien')
    writeFileSync(join(root, 'take.json'), '{}')

    const entries = await listRecordings(deps())

    expect(entries.map((entry) => entry.file)).toEqual(['take.mkv'])
  })

  it('descends into a dated folder', async () => {
    mkdirSync(join(root, '2026-10-30'))
    writeFileSync(join(root, '2026-10-30', 'take.mp4'), 'x')

    const entries = await listRecordings(deps())

    expect(entries.map((entry) => entry.file)).toEqual(['2026-10-30/take.mp4'])
  })
  ...
  it('does not judge a running take, and accuses it of nothing', async () => {
    ...
    const check = await inspectRecording(
      deps({ realNow: () => Date.now(), probe: probeFn }),
      'take.mkv',
    )
  ...
  it('does not break on a missing folder', async () => {
    expect(await listRecordings(deps({ root: join(root, 'jamais') }))).toEqual([])
  })
})

describe('technical check', () => {
  it('declares an empty file unreadable', async () => {
    writeFileSync(join(root, 'take.mkv'), '')

    const check = await inspectRecording(deps({ probe: probeFn() }), 'take.mkv')
  ...
  it('spots the gap between the stopwatch and the file', async () => {
    ...
    const check = await inspectRecording(
      deps({ probe: probeFn({ durationMs: 12 * 60_000 }) }),
      'take.mkv',
    )
  ...
  it('says the check is partial when ffprobe is missing', async () => {
    ...
    const check = await inspectRecording(deps({ probe: async () => null }), 'take.mkv')
...
describe('the operator\'s verdict', () => {
  it('survives the modal being closed and outranks the probe', async () => {
    video('take.mkv')
    sidecar('take.json')
    await inspectRecording(deps({ probe: probeFn() }), 'take.mkv')

    await setVerdict(deps(), 'take.mkv', 'illisible')
  ...
  it('can be cleared, to take back a slip', async () => {
    ...
    expect(await setVerdict(deps(), 'take.mkv', null)).toBeNull()
...
describe('a verdict going stale', () => {
  ...
  it('discards a verdict written before the fingerprint existed', async () => {
    ...
    writeFileSync(
      join(root, '.controles-vod.json'),
      JSON.stringify({
        version: 1,
        entries: {
          'take.mkv': { status: 'suspect', at: '2026-10-30T08:00:00.000Z', by: 'auto', reasons: ['sidecar absent'], probe: null },
        },
      }),
    )
...
describe('reading ffprobe', () => {
  it('keeps the tracks, the duration and the bitrate', () => {
    const probed = parseFfprobeOutput(
      JSON.stringify({
        streams: [
    ...
    expect(probed.durationMs).toBe(2_700_500)
    expect(probed.video?.fps).toBe(29.97)
    expect(probed.audio?.channels).toBe(2)
    expect(probed.bitrateKbps).toBe(8000)
  })
  ...
  it('falls back to the video track\'s duration', () => {
    ...
    expect(probed.durationMs).toBe(600_000)
    expect(probed.audio).toBeNull()
  })

  it('returns null when the tool is not installed', async () => {
    expect(await ffprobeProbe('ffprobe-that-does-not-exist')(join(root, 'take.mkv'))).toBeNull()
  })
  ...
  it('does not accuse the file when it is the probe that did not answer', async () => {
     ...
     */
    const fake = join(root, 'ffprobe-not-executable')
    writeFileSync(fake, '#!/bin/sh\nexit 0\n', { mode: 0o644 })
    video('take.mkv')
    sidecar('take.json')

    expect(await ffprobeProbe(fake)(join(root, 'take.mkv'))).toBeNull()

    const check = await inspectRecording(deps({ probe: ffprobeProbe(fake) }), 'take.mkv')
    expect(check.status).toBe('ok')
    expect(check.reasons.join(' ')).toContain('sonde ffprobe indisponible')
 ...
 */
async function makeRush(name: string): Promise<string | null> {
  if (!(await toolAvailable('ffmpeg'))) return null
  const path = join(root, name)
  const done = await new Promise<boolean>((settle) => {
    execFile(
      'ffmpeg',
 ...
 */
function closeStream(stream: { on: (e: string, h: () => void) => unknown; destroy: () => void }): void {
  stream.on('error', () => {})
  stream.destroy()
}
...
async function drain(stream: NodeJS.ReadableStream): Promise<Buffer> {
  const pieces: Buffer[] = []
  for await (const piece of stream) pieces.push(Buffer.from(piece as Buffer))
...
describe('reading footage', () => {
  it('serves the whole file when nothing is asked for', async () => {
    ...
    const stream = (await openFile(deps(), 'take.mkv'))!

    expect(stream.size).toBe(5_000)
    expect([stream.start, stream.end]).toEqual([0, 4_999])
    expect(stream.type).toBe('video/x-matroska')
  ...
  it('serves the requested range', async () => {
    ...
    video('take.mp4', 5_000)

    const stream = (await openFile(deps(), 'take.mp4', 'bytes=1000-1999'))!

    expect([stream.start, stream.end]).toEqual([1_000, 1_999])
    expect(stream.type).toBe('video/mp4')
    expect((await drain(stream.stream)).length).toBe(1_000)
  })

  it('serves the end of the file, where players look for the index', async () => {
    video('take.mkv', 5_000)

    const stream = (await openFile(deps(), 'take.mkv', 'bytes=-500'))!

    expect([stream.start, stream.end]).toEqual([4_500, 4_999])
    closeStream(stream.stream)
  })
...
describe('preview', () => {
  it('says no rather than open a player that will never start', async () => {
    ...
    expect(
      await openExcerpt(deps(), 'take.mkv', { command: 'ffmpeg-that-does-not-exist' }),
    ).toBeNull()
  })

  it('repackages a Matroska into MP4 without touching the file', async () => {
    const path = await makeRush('sample.mkv')
    if (path == null) return

    const excerptOpened = (await openExcerpt(deps({ probe: ffprobeProbe() }), 'sample.mkv', {
      atMs: 0,
      durationMs: 5_000,
    }))!
    const bytes = await drain(excerptOpened.stream)
    excerptOpened.stop()
